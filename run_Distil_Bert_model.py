...
from models import Distil_BertModel as DistilBertModel
...
def model_train_validate_test(train_df, dev_df, test_df, target_dir, 
    ...
    distil_bertmodel = DistilBertModel(requires_grad = True)
    tokenizer = distil_bertmodel.tokenizer
    ...
    model = distil_bertmodel.to(device)
    ...
    print("\n", 20 * "=", "Training Distil-bert model on device: {}".format(device), 20 * "=")
    patience_counter = 0
...
def model_load_test(test_df, target_dir, test_prediction_dir, test_prediction_name, max_seq_len=50, batch_size=32):
    ...
    """
    distil_bertmodel = DistilBertModel(requires_grad = False)
    tokenizer = distil_bertmodel.tokenizer
    device = torch.device("cuda")
