...
def Metric(y_true, y_pred):
    ...
    macro_precision = precision_score(y_true, y_pred.prediction, average='macro')
    macro_recall = recall_score(y_true, y_pred.prediction, average='macro')
    weighted_f1 = f1_score(y_true, y_pred.prediction, average='macro')
    target_names = ['class_0', 'class_1']
    report = classification_report(y_true, y_pred.prediction, target_names=target_names, digits=3)
    ####
    predicted_prob=[]
    for i, j in zip(y_pred.prob_0,y_pred.prob_1):
      predicted_prob.append((i,j))
    predicted_prob=np.array(predicted_prob)
    predicted_prob_max= np.amax(predicted_prob, axis=1)

    auc = metrics.roc_auc_score(np.array(y_true), predicted_prob_max,multi_class="ovr")

    ####
    print('Accuracy: {:.1%}\nPrecision: {:.1%}\nRecall: {:.1%}\nF1: {:.1%}\nAUC: {}'.format(accuracy, macro_precision,
                                           macro_recall, weighted_f1,round(auc,2)))
